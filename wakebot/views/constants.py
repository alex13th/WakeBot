...
class ReserveTelegramStringsRu:
    ...
    hello_message = "Главное меню сервиса бронирований"
    ...
    phone_error_message = (
        ...
        f"\n\n{phone_message}")
    phone_warning = f"{icon_warning} Обязательно укажите номер телефона."
    date_format = "%d.%m.%Y"
    date_text = "Дата"
    ...
    cancel_button = f"{icon_cancel} Отменить {book_text.lower()}"
    cancel_button_callback = f"{book_text} отменено"
    cancel_notify_header = f"{icon_cancel} *Отменено {book_text.lower()}*"
    ...
    apply_button = f"{icon_ok} {apply_text}"
...
class ReserveTelegramStringsEng:
    icon_stop = "⛔️"
    icon_cancel = "❌"
    icon_phone = "📞"
    icon_warning = "⚠️"
    ...
    phone_error_message = (
        ...
        f"\n\n{phone_message}")
    phone_warning = f"{icon_warning} {phone_text} required."
    date_format = "%d.%m.%Y"
    date_text = "Date"
    date_label = f"*{date_text}:*"
    ...
    cancel_button = f"{icon_cancel} Cancel {booking_text.lower()}"
    cancel_button_callback = f"{booking_text} is canceled"
    cancel_notify_header = f"{icon_cancel} *{booking_text} is canceled*"
