...
from wakebot.adapters.sqlite import SqliteBathhouseAdapter, SqliteUserAdapter
from wakebot.adapters.state import StateManager
from wakebot.processors import RuBathhouse, BathhouseProcessor
from wakebot.entities import Bathhouse


class BathhouseProcessorTestCase(ReserveProcessorTestCase):
    "BathhouseProcessor class"

    def setUp(self):
        super().setUp()
        self.strings = RuBathhouse

        self.dp = Dispatcher()
        self.data_adapter = MemoryDataAdapter()
        self.state_manager = StateManager(self.data_adapter)
        self.processor = BathhouseProcessor(self.dp,
    ...
    def prepare_data(self):
        ...
        self.bathhouse_adapter = SqliteBathhouseAdapter(self.connection)
        ...
        for i in range(8):
            ...
            bathhouse = Bathhouse(user, start_date=start_date,
                                  ...
                                  set_count=(i + 1))
            bathhouse.count = i % 3
            bathhouse = self.bathhouse_adapter.append_data(bathhouse)
            self.reserves.append(bathhouse)
        self.processor = BathhouseProcessor(self.dp, self.state_manager,
                                            self.strings,
                                            self.bathhouse_adapter,
                                            self.user_adapter)
        self.processor.max_count = 6
    ...
    def create_book_keyboard(self):
        ...
        if self.state_manager.data:
            reserve: Bathhouse = self.state_manager.data
    ...
    async def test_cmd_booking(self):
        """Proceed /booking command"""
        message = self.test_message
        message.text = "/booking"
        reply_markup = self.create_main_keyboard()

        await self.processor.cmd_booking(message)
    ...
    async def test_callback_book_apply(self):
        ...
        reserve = self.state_manager.data
        reserve = self.bathhouse_adapter.get_data_by_keys(reserve.id)
...
if __name__ == "__main__":
    BathhouseProcessorTestCase().run_tests_async()
